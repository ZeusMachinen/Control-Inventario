...
const ComparativaRebanos = {
  modo: 'anios', // 'periodos' | 'anios' | 'rebanos'
  ...
  async render(container) {
    ...
    container.innerHTML = `
      <div class="d-flex gap-2 mb-3">
        <button class="btn btn-sm ${this.modo==='periodos'?'btn-primary':'btn-outline-primary'}" onclick="ComparativaRebanos.cambiarModo('periodos')"><i class="fas fa-calendar-alt me-1"></i>Mes vs Mes</button>
        <button class="btn btn-sm ${this.modo==='anios'?'btn-primary':'btn-outline-primary'}" onclick="ComparativaRebanos.cambiarModo('anios')"><i class="fas fa-calendar me-1"></i>Año vs Año</button>
        <button class="btn btn-sm ${this.modo==='rebanos'?'btn-primary':'btn-outline-primary'}" onclick="ComparativaRebanos.cambiarModo('rebanos')"><i class="fas fa-layer-group me-1"></i>Rebano vs Rebano</button>
      </div>
  ...
  async cargarContenido() {
    ...
    } else if (this.modo === 'anios') {
      await this.renderAnios(content);
    } else {
  ...
  async comparar() {
    ...
    try {
      ...
      const [resA, resB] = await Promise.all([
        API.get(`/analitica/historico-kpis?fecha_desde=${desdeA}&fecha_hasta=${hastaA}${qp}`),
        API.get(`/analitica/historico-kpis?fecha_desde=${desdeB}&fecha_hasta=${hastaB}${qp}`)
      ]);
      ...
      const filas = [
        ...
        ['Altas (nac+comp)', a.altas, b.altas],
        ['Nacidos en Finca', a.nacidos_finca, b.nacidos_finca],
        ['Comprados', a.comprados, b.comprados],
        ['Bajas (vend+muertos)', a.bajas, b.bajas],
        ['Partos en periodo', a.partos, b.partos],
        ...
        ['Gastos', Formateador.moneda(a.gastos), Formateador.moneda(b.gastos)],
      ];
  ...
  async renderAnios(content) {
    ...
    content.innerHTML = `
      <div class="comparativa-periodos row g-2 align-items-end mb-4">
        <div class="col-auto"><label class="form-label small">Año A</label>
          <select id="cmp-anio-a" class="form-select form-select-sm" onchange="ComparativaRebanos.compararAnios()" style="width:100px">${anos.replace(`value="${anioActual}"`,`value="${anioActual}" selected`)}</select></div>
        <div class="col-auto"><span class="fs-4">vs</span></div>
        <div class="col-auto"><label class="form-label small">Año B</label>
          <select id="cmp-anio-b" class="form-select form-select-sm" onchange="ComparativaRebanos.compararAnios()" style="width:100px">${anos.replace(`value="${anioActual}"`,`value="${anioActual-1}" selected`)}</select></div>
        <div class="col-auto"><button class="btn btn-primary btn-sm" onclick="ComparativaRebanos.compararAnios()"><i class="fas fa-balance-scale me-1"></i>Comparar</button></div>
      </div>
      <div id="comparativa-resultado"><p class="text-muted">Selecciona dos años y hace clic en Comparar.</p></div>
    `;
  },
  ...
  async compararAnios() {
    ...
    const desdeA = `${anioA}-01-01`;
    const hastaA = `${anioA}-12-31`;
    const desdeB = `${anioB}-01-01`;
    const hastaB = `${anioB}-12-31`;

    const resultado = document.getElementById('comparativa-resultado');
    if (!resultado) return;
    resultado.innerHTML = '<div class="text-center py-3"><i class="fas fa-spinner fa-spin"></i> Comparando años...</div>';
    ...
    try {
      ...
      const filas = [
        ...
        ['Comprados', a.comprados, b.comprados],
        ['Bajas (vend+muertos)', a.bajas, b.bajas],
        ['Partos en el año', a.partos, b.partos],
        ['Tasa Natalidad', (a.tasa_natalidad||0)+'%', (b.tasa_natalidad||0)+'%'],
        ['Ganancia Neta', Formateador.moneda(a.ganancia_neta), Formateador.moneda(b.ganancia_neta)],
        ['Gastos', Formateador.moneda(a.gastos), Formateador.moneda(b.gastos)],
      ];
      ...
      resultado.innerHTML = `
        <div class="table-responsive">
          <table class="table table-sm">
            <thead><tr><th>KPI</th><th>${anioA}</th><th>${anioB}</th><th>Variación</th></tr></thead>
            <tbody>${filas.map(f => `<tr>
              <td><strong>${f[0]}</strong></td><td>${f[1]??'-'}</td><td>${f[2]??'-'}</td>
              <td>${variacion(f[1], f[2])}</td></tr>`).join('')}</tbody>
          </table>
        ...
        <div class="table-responsive">
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              ...
              <tr><td><strong>Riesgo Descarte</strong></td>${rebanos.map(r => `<td><span class="badge ${(r.riesgo_descarte?.pct_riesgo||0)>15 ? 'cat-badge cat-badge-descarte-atencion' : 'cat-badge cat-badge-descarte-bajo'}">${r.riesgo_descarte?.pct_riesgo||0}%</span></td>`).join('')}</tr>
