const RebanoDetailPage = {
    ...
    try {
      ...
      return MainLayout.render(`
        <div class="page-header">
          ...
          <button class="btn btn-outline-secondary" onclick="Router.navegar('/rebanos')"><i class="fas fa-arrow-left me-1"></i>Volver</button>
        </div>

        <div class="d-flex gap-3 flex-wrap align-items-center mb-3">
          <span class="small text-secondary"><i class="fas fa-calendar me-1"></i>Inicio: ${fechaInicio}</span>
          <span class="badge ${r.activo ? 'bg-success' : 'bg-danger'}">${r.activo ? 'Activo' : 'Inactivo'}</span>
          ${this.costoPorCabeza ? `<span class="small text-secondary"><i class="fas fa-coins me-1"></i>Costo/cabeza: $${this.costoPorCabeza.toFixed(2)}</span>` : ''}
          ${r.dia_corte ? `<span class="small text-secondary"><i class="fas fa-scissors me-1"></i>Corte: día ${r.dia_corte}</span>` : ''}
          ${r.dia_corte && this.costoPorCabeza ? `<button class="btn btn-outline-primary btn-sm" onclick="RebanoDetailPage.generarPastaje(${r.id}, this)"><i class="fas fa-money-bill-wave me-1"></i>Generar gasto pastaje</button>` : ''}
        </div>

        <div class="filter-panel">
          <div class="mb-0">
            <label class="form-label">Período</label>
            <select class="form-select form-select-sm" id="detail-filtro-modo" onchange="RebanoDetailPage.cambiarModo()" style="min-width:160px">
          ...
          <div class="mb-0" id="detail-filtro-mes-group" style="display:none">
            <label class="form-label">Mes</label>
            <input type="month" class="form-control form-control-sm" id="detail-filtro-mes" value="${this.mesActual}" onchange="RebanoDetailPage.cargarEstadisticas()">
          </div>
          <div class="mb-0" id="detail-filtro-anio-group" style="display:none">
            <label class="form-label">Año</label>
            <select class="form-select form-select-sm" id="detail-filtro-anio" onchange="RebanoDetailPage.cargarEstadisticas()">
              ${this.generarOpcionesAnio()}
            </select>
          </div>
          <div class="mb-0" id="detail-filtro-desde-group" style="display:none">
            <label class="form-label">Desde</label>
            <input type="date" class="form-control form-control-sm" id="detail-filtro-desde" onchange="RebanoDetailPage.cargarEstadisticas()">
          </div>
          <div class="mb-0" id="detail-filtro-hasta-group" style="display:none">
            <label class="form-label">Hasta</label>
            <input type="date" class="form-control form-control-sm" id="detail-filtro-hasta" onchange="RebanoDetailPage.cargarEstadisticas()">
          </div>
        </div>
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card" style="border-left-color:var(--naranja)">
            <div class="stat-card-icon" style="background:var(--naranja-claro);color:var(--naranja)"><i class="fas fa-sack-dollar"></i></div>
            <div class="stat-card-info">
          ...
          <div class="stat-card" style="border-left-color:#2E7D32;cursor:pointer" onclick="Router.navegar('/rebanos/${r.id}/movimientos')">
            <div class="stat-card-icon" style="background:#E8F5E9;color:#2E7D32"><i class="fas fa-right-left"></i></div>
            <div class="stat-card-info">
          ...
          <div class="stat-card" style="border-left-color:#1565C0;cursor:pointer" onclick="Router.navegar('/rebanos/${r.id}/costos')">
            <div class="stat-card-icon" style="background:#E3F2FD;color:#1565C0"><i class="fas fa-coins"></i></div>
            <div class="stat-card-info">
          ...
          <div class="stat-card" style="border-left-color:#6A1B9A">
            <div class="stat-card-icon" style="background:#F3E5F5;color:#6A1B9A"><i class="fas fa-sack-dollar"></i></div>
            <div class="stat-card-info">
              <h3 id="kpi-valor">···</h3>
        ...
        <div class="card mb-4">
          <div class="card-header d-flex justify-content-between align-items-center">
            <strong><i class="fas fa-horse me-2"></i>Animales de este Rebaño</strong>
        ...
        <div class="card">
          <div class="card-header"><strong><i class="fas fa-chart-bar me-2"></i>Estadísticas Detalladas</strong></div>
          <div class="card-body">
            <div class="row g-2" style="max-width:400px">
              <div class="col-6 fw-bold">Kg Producidos:</div>
              <div class="col-6" id="detail-kilos">···</div>
              <div class="col-6 fw-bold">Ingresos Generados:</div>
              <div class="col-6" id="detail-ingresos">···</div>
            </div>
          </div>
        </div>
      `);
  ...
  async generarPastaje(id, btnEl) {
    try {
      if (btnEl) { btnEl.disabled = true; btnEl.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Generando...'; }
      await API.post(`/rebanos/${id}/generar-pastaje`);
      Toast.success('Gasto de pastaje generado/actualizado. Revisá la sección Gastos.');
    } catch (err) {
      Toast.error(err.response?.data?.error || 'Error al generar gasto de pastaje');
    } finally {
      if (btnEl) { btnEl.disabled = false; btnEl.innerHTML = '<i class="fas fa-money-bill-wave me-1"></i>Generar gasto pastaje'; }
    }
  },
  ...
  async cargarAnimales() {
    ...
    try {
      ...
      if (animales.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-secondary">No hay animales en este rebaño</td></tr>';
        return;
      }
      tbody.innerHTML = animales.map(a => `
        <tr>
          ...
          <td>${a.estado_reproductivo ? `<span class="badge ${a.estado_reproductivo === 'Prenada' ? 'bg-warning' : 'bg-primary'}">${a.estado_reproductivo}</span>` : '-'}</td>
          <td>
            <button class="btn btn-outline-secondary btn-sm" onclick="Router.navegar('/animales/${a.id}')"><i class="fas fa-eye"></i></button>
          </td>
        </tr>
      `).join('');
    } catch (e) {
      tbody.innerHTML = `<tr><td colspan="6" class="text-danger py-3 text-center">Error al cargar animales</td></tr>`;
    }
  },
};
